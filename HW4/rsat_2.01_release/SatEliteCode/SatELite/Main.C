...
static cchar* doc =
  "RSat 2.01 (+preprocessor) options:\n"
  "\n"
  " -s          \tsolution. Print out solution if one is found.\n"
  " -t <timeout>\ttime-out. Stop and return UNKNOWN after <timeout> seconds. (Only applicable to the solver part)\n"
  " -v          \tverbose. Print out useful information during execution of the solver.\n"
  "\n"
  "Example:\n"
  "\t./rsat.sh problem.cnf -s -t 100 -v\n"
...
void parseOptions(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++){
	    ...
	    //added by Knot 6/4/07
	    else if (strcmp(arg+1, "s") == 0) opt_print_solution = true;
	    else if (strcmp(arg+1, "t") == 0) i++; //timeout option for RSat, skip the next argument
	    else if (strcmp(arg+1, "v") == 0) ; //verbose option for RSat, skip it
	    else if (strcmp(arg+1, "q") == 0) ; //quiet option for RSat, skip it
	      ...
	      //eventually, we may want to just ignore unrecognized options
...
template<class B>
static void readLine(char* line,B& in)
{
  for(int i=0;;){
    if(*in==EOF) return;
    if(*in=='\n') {++in; return;}
...
static void parse_DIMACS_main(B& in, Solver& S) {
    ...
    for (;;){
	  ...
	  if(num_clause>4800000){
	    printf("c num clause = %d, num vars = %d\n",num_clause,num_var);
...
int main(int argc, char** argv)
{
    ...
    if (!opt_ext_sat){
	  ...
	  //Knot 10/30/06 printf("s UNSATISFIABLE\n");
            exit(20);
	...
	//Knot added print solution as an option (6/4/07)
